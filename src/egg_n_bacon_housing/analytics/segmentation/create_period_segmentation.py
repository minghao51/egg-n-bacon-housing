        ...
        for ptype in period_data["property_type"].unique():
            ...
            for tier in ["Mass Market", "Mid-Tier", "Luxury"]:
                ...
                if count > 0:
                    ...
                    logger.info(
                        f"    {tier}: {count:,} ({pct:.1f}%) - Median: ${tier_prices.median():,.0f}"
                    )
...
def analyze_tier_evolution(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, row in hdb_thresholds.iterrows():
        logger.info(
            f"  {row['period']}: Mass ≤${row['mass_market_max']:,.0f} | "
            f"Mid ${row['mass_market_max']:,.0f}-${row['mid_tier_max']:,.0f} | "
            f"Luxury ≥${row['luxury_min']:,.0f}"
        )

    logger.info("\nTier Threshold Evolution (Condominium):")
    condo_thresholds = threshold_df[threshold_df["property_type"] == "Condominium"].sort_values(
        "period"
    )
...
def compare_old_vs_new_segmentation(df: pd.DataFrame):
    ...
    if "market_tier_period" in df.columns and "market_tier" in df.columns:
        comparison = (
            df.groupby(["property_type", "market_tier", "market_tier_period"])
            .size()
        ...
        for _, row in transitions.iterrows():
            logger.info(
                f"  {row['property_type']}: {row['market_tier']} → {row['market_tier_period']}: "
...
def save_segmentation_results(df: pd.DataFrame, thresholds: pd.DataFrame, output_dir: Path):
    ...
    recent_thresholds = (
        thresholds.sort_values("period").groupby("property_type").last().reset_index()
    )
    ...
    if "era" in df.columns:
        ...
        era_summary = (
            df.groupby(["era", "property_type"])
            .agg({"price": ["count", "median", "mean"], "price_psf": "median"})
            .reset_index()
        )
        era_summary.columns = [
            "era",
    ...
    if "comparison_era" in df.columns:
        ...
        comparison_summary = (
            df.groupby(["comparison_era", "property_type"])
            .agg({"price": ["count", "median", "mean"], "price_psf": "median"})
            .reset_index()
        )
...
def main():
    ...
    l3_path = settings.l3_dir / "housing_unified.parquet"
    ...
    # Save results
    output_dir = settings.DATA_DIR / "analysis" / "market_segmentation_period"
