...
def calculate_psf_tiers(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for ptype in df["property_type"].unique():
        ...
        for tier in ["Low PSF", "Medium PSF", "High PSF"]:
            ...
            logger.info(
                f"  {tier}: {count:,} ({pct:.1f}%) - Median: ${tier_psf.median():,.0f}/sqft"
            )
...
def analyze_tier_performance(df: pd.DataFrame) -> pd.DataFrame:
    ...
    performance = (
        df.groupby(["property_type", "market_tier", "year"])
        .agg({"price": ["median", "count"], "price_psf": "median"})
        .reset_index()
    )
...
def main():
    ...
    # Save results
    output_dir = settings.DATA_DIR / "analysis" / "market_segmentation"
