...
def calculate_estimated_income(planning_area: str, hdb_prices: dict, flat_type_dist: dict) -> dict:
    """
    ...
    combined_factor = 0.50 * price_factor + 0.30 * mrt_factor + 0.20 * flat_type_factor
    ...
    return {
        ...
        "estimated_median_annual_income": round(
            estimated_annual_income, -3
...
def get_hdb_prices_by_planning_area() -> dict:
    ...
    try:
        hdb = pd.read_parquet("data/pipeline/02_silver/housing_hdb_transaction.parquet")
...
def generate_all_estimates() -> pd.DataFrame:
    ...
    for pa in planning_areas:
        flat_dist = {"large_flat_pct": flat_type_dist.get(pa, 0.30)}

        estimate = calculate_estimated_income(pa, hdb_prices, flat_dist)
        results.append(estimate)

        print(
            f"  {pa:20s}: ${estimate['estimated_median_annual_income']:,.0f}/year "
...
def main():
    ...
    output_dir = Path("data/pipeline/02_silver")
    ...
    print("=" * 60)
    print(
        f"Median across planning areas: ${df['estimated_median_annual_income'].median():,.0f}/year"
    )
    print(
        f"Range: ${df['estimated_median_annual_income'].min():,.0f} - ${df['estimated_median_annual_income'].max():,.0f}"
    )
