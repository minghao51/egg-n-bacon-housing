...
def load_base_data() -> pd.DataFrame:
    ...
    l3_path = Path("data/pipeline/04_platinum/housing_unified.parquet")

    if not l3_path.exists():
        # Try the period-segmented version
        segmented_path = Path(
            "data/analytics/market_segmentation_period/housing_unified_period_segmented.parquet"
        )
...
def identify_coming_soon_properties(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "transaction_date" in df.columns and not pd.api.types.is_datetime64_any_dtype(
        df["transaction_date"]
    ):
    ...
    recent_activity = (
        df[df["transaction_date"] >= (current_date - timedelta(days=180))].groupby("town").size()
    )
...
def calculate_forecasted_metrics(df: pd.DataFrame) -> list[ForecastedMetric]:
    ...
    for pa in planning_areas:
        ...
        if "transaction_date" in pa_df.columns:
            monthly = (
                pa_df.groupby(pd.to_datetime(pa_df["transaction_date"]).dt.to_period("M"))
                .agg(
                    {
                        "price": "median",
                        "price_psf": "median",
...
def calculate_investment_score(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "mom_change_pct" in df.columns:
        ...
        df["momentum_score"] = (
            (momentum - momentum.min()) / (momentum.max() - momentum.min())
        ) * 30  # 30% weight
    else:
    ...
    if "dist_to_nearest_mrt" in df.columns:
        ...
        df["infra_score"] = (
            (mrt_dist.max() - mrt_dist) / (mrt_dist.max() - mrt_dist.min())
        ) * 20  # 20% weight
    else:
        df["infra_score"] = 10
    ...
    if amenity_cols:
        ...
        df["amenity_score"] = (
            (amenity_score - amenity_score.min()) / (amenity_score.max() - amenity_score.min())
        ) * 10  # 10% weight
    else:
        df["amenity_score"] = 5
    ...
    df["investment_score"] = (
        df["yield_score"] + df["momentum_score"] + df["infra_score"] + df["amenity_score"]
    )

    logger.info(
        f"Investment scores calculated (range: {df['investment_score'].min():.1f} - {df['investment_score'].max():.1f})"
    )
...
def generate_summary_report(
    ...
    if forecasts:
        ...
        for horizon in ["6m", "12m"]:
            ...
            if not horizon_df.empty:
                ...
                logger.info(
                    f"    Range: {horizon_df['trend_pct'].min():+.1f}% to {horizon_df['trend_pct'].max():+.1f}%"
                )
                ...
                for _, row in top_gainers.iterrows():
                    logger.info(
                        f"      - {row['planning_area']}: +{row['trend_pct']:.1f}% to ${row['predicted_value']:,.0f}"
                    )
    ...
    if comparisons:
        ...
        if not covid_impact.empty:
            ...
            logger.info(
                f"    Most affected: {comparison_df.loc[covid_impact.idxmin(), 'planning_area']} ({covid_impact.min():+.1f}%)"
            )
            logger.info(
                f"    Least affected: {comparison_df.loc[covid_impact.idxmax(), 'planning_area']} ({covid_impact.max():+.1f}%)"
            )
        ...
        if not recovery.empty:
            ...
            logger.info(
                f"    Best recovery: {comparison_df.loc[recovery.idxmax(), 'planning_area']} ({recovery.max():+.1f}%)"
            )
...
def main():
    ...
    output_dir = Path("data/analytics/coming_soon")
    output_dir.mkdir(parents=True, exist_ok=True)
