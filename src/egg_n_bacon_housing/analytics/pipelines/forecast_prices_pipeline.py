...
def load_price_data(property_type: str = "HDB") -> pd.DataFrame:
    ...
    """
    path = Path("data/pipeline/04_platinum/housing_unified.parquet")
...
def prepare_prophet_data(
    df: pd.DataFrame, planning_area: str, price_column: str = "price"
) -> pd.DataFrame:
...
def generate_forecast(model: Prophet, horizon_months: int, last_date: pd.Timestamp) -> pd.DataFrame:
...
def calculate_trend_pct(last_actual: float, forecast_value: float) -> float:
...
def forecast_all_planning_areas(
    property_type: str = "HDB", horizons: list[int] = [6, 12]
...
def main():
    ...
    results, results_df = forecast_all_planning_areas(property_type="HDB", horizons=[6, 12])
