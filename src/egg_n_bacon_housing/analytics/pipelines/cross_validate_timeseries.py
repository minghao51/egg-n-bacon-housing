# egg_n_bacon_housing/analytics/pipelines/cross_validate_timeseries.py
"""
...
Usage:
    from egg_n_bacon_housing.analytics.pipelines.cross_validate_timeseries import run_cross_validation
...
from egg_n_bacon_housing.analytics.models.area_arimax import AreaARIMAXModel
from egg_n_bacon_housing.analytics.models.regional_var import RegionalVARModel
...
def evaluate_model_performance(actual: pd.Series, forecast: pd.Series) -> dict[str, float]:
    ...
    if mask.sum() > 0:
        mape = (
            np.mean(np.abs((aligned_actual[mask] - aligned_forecast[mask]) / aligned_actual[mask]))
            * 100
        )
    else:
        mape = np.nan
...
def run_rolling_validation(
    data: pd.DataFrame, region: str, n_folds: int = 5, forecast_horizon: int = 12
) -> dict:
    """
    ...
    """
    logger.info(
        f"Running rolling validation for {region}: {n_folds} folds, {forecast_horizon}-month horizon"
    )
    ...
    logger.info(
        f"Validation complete: RMSE={mean_rmse:.2f}, MAE={mean_mae:.2f}, DirAcc={mean_directional:.1f}%"
    )
...
def run_cross_validation(
    regional_data: pd.DataFrame, area_data: pd.DataFrame, n_folds: int = 5
) -> dict:
    """
    ...
    for region in regional_data["region"].unique():
        ...
        region_results = run_rolling_validation(
            data=region_data, region=region, n_folds=n_folds, forecast_horizon=12
        )
