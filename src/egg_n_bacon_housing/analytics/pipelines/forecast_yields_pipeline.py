...
def load_rental_yield_data() -> pd.DataFrame:
    ...
    path = Path("data/pipeline/03_gold/rental_yield.parquet")
...
def prepare_prophet_data(df: pd.DataFrame, planning_area: str) -> pd.DataFrame:
...
def calculate_trend_bps(last_actual: float, forecast_value: float) -> float:
    """Calculate change in basis points."""
    return (forecast_value - last_actual) * 100  # Convert % to bps


def forecast_yields(planning_area: str, horizons: list = [6, 12]) -> list:
...
def main():
    ...
    if len(results_df) > 0:
        ...
        print(
            f"  Range: {df_6m['predicted_yield_pct'].min():.2f}% to {df_6m['predicted_yield_pct'].max():.2f}%"
        )
