...
def load_data() -> tuple:
    ...
    hdb_path = Path("data/pipeline/02_silver/housing_hdb_transaction.parquet")
    hdb = pd.read_parquet(hdb_path)

    # Load income estimates
    income_path = Path("data/pipeline/02_silver/household_income_estimates.parquet")
    income = pd.read_parquet(income_path)

    # Load rental yields (optional, for yield-adjusted affordability)
    rental_path = Path("data/pipeline/03_gold/rental_yield.parquet")
...
def calculate_affordability_metrics(
    hdb: pd.DataFrame, income: pd.DataFrame, rental: pd.DataFrame = None
) -> pd.DataFrame:
    ...
    affordability = price_by_pa.merge(
        income.rename(columns={"planning_area": "town"})[
            ["town", "estimated_median_annual_income", "estimated_median_monthly_income"]
        ],
        on="town",
        how="left",
    )
    ...
    affordability["mortgage_to_income_pct"] = (
        ...
        * 12
        / affordability["estimated_median_annual_income"]
        * 100
    )
    ...
    if rental is not None:
        ...
        affordability["gross_yield_pct"] = (
            affordability["avg_rental_yield"] * 12 / affordability["median_price"]
        ) * 100
...
def generate_summary(affordability: pd.DataFrame) -> None:
    ...
    print(
        f"Range: {affordability['affordability_ratio'].min():.2f} - {affordability['affordability_ratio'].max():.2f}"
    )
    ...
    most_affordable = affordability.head(5)[
        [
            ...
            "affordability_ratio",
            "mortgage_to_income_pct",
        ]
    ]
    for _, row in most_affordable.iterrows():
        print(
            f"  {row['town']:20s}: Ratio {row['affordability_ratio']:.2f}, "
...
def create_affordability_by_month(affordability: pd.DataFrame, hdb: pd.DataFrame) -> pd.DataFrame:
    ...
    # Load income estimates
    income = pd.read_parquet("data/pipeline/02_silver/household_income_estimates.parquet")
...
def main():
    ...
    output_dir = Path("data/pipeline/04_platinum")
