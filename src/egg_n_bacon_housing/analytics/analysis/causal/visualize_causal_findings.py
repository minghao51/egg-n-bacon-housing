...
def plot_did_analysis(output_dir: Path):
    ...
    ccr_prices = [
        did_results["treatment_pre_price"].values[0],
        did_results["treatment_post_price"].values[0],
    ]
    ocr_prices = [
        did_results["control_pre_price"].values[0],
        did_results["control_post_price"].values[0],
    ]
    ...
    ax1.set_title(
        "DiD Analysis: Sep 2022 ABSD Changes\nCondominium CCR vs OCR",
        fontweight="bold",
        fontsize=14,
    )
    ax1.set_xticks(x)
    ...
    ax1.annotate(
        f"DiD Estimate: -${did_estimate / 1e3:.0f}K\n(p = {p_value:.3f} **)",
    ...
    changes_pct = [
        did_results["treatment_change"].values[0]
        / did_results["treatment_pre_price"].values[0]
        * 100,
...
def plot_rd_analysis(output_dir: Path):
    ...
    ax1.plot(
        ...
        linewidth=2,
        markersize=8,
        label="HDB Median Price",
    )
    ax1.axvline(x=0, color="#e74c3c", linestyle="--", linewidth=2, label="Dec 2023 Policy")
    ax1.axhline(
        y=pre_mean, color="#3498db", linestyle=":", linewidth=1, alpha=0.5, label="Pre-policy mean"
    )
    ax1.axhline(
        y=post_mean,
        color="#9b59b6",
    ...
    ax2.set_title(
        "RDiT: Slope Kink at Policy\nPre: -$551/mo → Post: +$3,212/mo",
        fontweight="bold",
        fontsize=14,
    )
    ...
    ax2.annotate(
        f"Slope Change: +${kink_coef:,.0f}/month\n(+{slope_change_pct:.0f}% acceleration)",
...
def plot_lease_arbitrage(output_dir: Path):
    ...
    ax1.plot(
        viz_df["lease_years"],
        viz_df["market_value_pct"],
        "o-",
        linewidth=2,
        markersize=6,
        label="Market Value (Spline)",
        color="#3498db",
    )
    ax1.plot(
        viz_df["lease_years"],
        viz_df["theoretical_value_pct"],
        "--",
        linewidth=2,
        label="Bala's Theoretical",
        color="#e74c3c",
    )
    ...
    ax1.fill_between(
        ...
        viz_df["market_value_pct"],
        viz_df["theoretical_value_pct"],
        where=sell_mask.values,
        alpha=0.3,
        color="green",
        label="Overvalued (SELL)",
    )
    ax1.fill_between(
        viz_df["lease_years"],
        viz_df["market_value_pct"],
        viz_df["theoretical_value_pct"],
        where=buy_mask.values,
        alpha=0.3,
        color="orange",
        label="Undervalued (BUY)",
    )
...
def main():
    ...
    output_dir = settings.DATA_DIR / "analysis"
