...
def load_property_data() -> pd.DataFrame:
    ...
    possible_paths = [
        settings.l3_dir / "housing_unified.parquet",
        settings.l2_dir / "housing_multi_amenity_features.parquet",
        settings.DATA_DIR
        / "analysis"
        / "market_segmentation"
        / "housing_unified_segmented.parquet",
    ]
...
def create_cluster_profiles(
    df: pd.DataFrame, labels: np.ndarray, feature_cols: list
) -> pd.DataFrame:
    ...
    cluster_profiles = (
        df_result[df_result["cluster_id"] >= 0]
        .groupby("cluster_id")
        .agg(
            {
...
def summarize_results(
    labels: np.ndarray, silhouette_score: float, profiles: pd.DataFrame, names: dict
) -> list:
...
def main():
    ...
    output_dir = settings.ANALYSIS_OUTPUT_DIR / "analyze_h3_clusters"
