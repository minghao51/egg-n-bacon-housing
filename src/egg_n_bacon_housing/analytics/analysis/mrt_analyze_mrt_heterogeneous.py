...
def analyze_by_flat_type(hdb):
    ...
    for flat_type in hdb["flat_type"].unique():
        ...
        # Prepare features
        X = subset[
            ["dist_to_nearest_mrt", "floor_area_sqm", "remaining_lease_months", "year"]
        ].values
    ...
    print(
        results_df[["flat_type", "n", "mean_price", "mrt_coef_100m", "r2"]].to_string(index=False)
    )
...
def analyze_by_price_tier(hdb):
    ...
    hdb["price_tier"] = pd.qcut(
        hdb["price_psf"], q=4, labels=["Budget", "Mid-Budget", "Mid-Premium", "Premium"]
    )
    ...
    print(
        results_df[["price_tier", "n", "mean_price", "price_range", "mrt_coef_100m"]].to_string(
            index=False
        )
    )
...
def create_town_premiums_ranked_chart(town_results):
    ...
    bars = ax.barh(
        range(len(town_sorted)), premiums, color=colors, edgecolor="black", linewidth=0.5
    )
    ...
    ax.set_title("MRT Premium by Town (HDB Properties, 2021+)", fontsize=14, fontweight="bold")
    ...
    ax.text(
        0.98,
        0.02,
        f"Highest: {max_town} (+${max_val:.1f}/100m)\nLowest: {min_town} (${min_val:.1f}/100m)",
...
def main():
    ...
    print("\nBy Flat Type:")
    print(
        f"  Range: ${flat_type_results['mrt_coef_100m'].min():.2f} to ${flat_type_results['mrt_coef_100m'].max():.2f} per 100m"
    )
    print(
        f"  Most sensitive: {flat_type_results.iloc[0]['flat_type']} (${abs(flat_type_results.iloc[0]['mrt_coef_100m']):.2f})"
    )

    print("\nBy Town:")
    print(
        f"  Range: ${town_results['mrt_coef_100m'].min():.2f} to ${town_results['mrt_coef_100m'].max():.2f} per 100m"
    )
    print(
        f"  Most sensitive: {town_results.iloc[0]['town']} (${abs(town_results.iloc[0]['mrt_coef_100m']):.2f})"
    )
