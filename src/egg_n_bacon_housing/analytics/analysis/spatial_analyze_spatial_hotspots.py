...
def load_rental_data() -> pd.DataFrame:
    ...
    # Load L1 rental data
    rental_path = settings.l1_dir / "housing_hdb_rental.parquet"
    ...
    geo_path = settings.l2_dir / "housing_unique_searched.parquet"
    ...
    rental_df["join_key"] = (
        rental_df["block"].astype(str) + "_" + rental_df["street_name"].str.upper()
    )
...
def aggregate_to_h3(df: pd.DataFrame, resolution: int = 8) -> pd.DataFrame:
    ...
    aggregated = (
        df.groupby("h3_index")
        .agg({"monthly_rent": ["median", "mean", "count"], "lat": "mean", "lon": "mean"})
        .reset_index()
    )
...
def compute_gistar(aggregated_df: pd.DataFrame, k_neighbors: int = 8) -> pd.DataFrame:
    ...
    df["classification"] = df.apply(
        lambda row: classify_gi(row["gi_star"], row["gi_pvalue"]), axis=1
    )
...
def summarize_results(df: pd.DataFrame) -> list:
    ...
    if len(hotspots) > 0:
        ...
        findings.append(
            f"Top hotspot: {top_hotspot['h3_index']} (Gi*={top_hotspot['gi_star']:.2f})"
        )

    if len(coldspots) > 0:
        top_coldspot = coldspots.nsmallest(1, "gi_star").iloc[0]
        findings.append(
            f"Top coldspot: {top_coldspot['h3_index']} (Gi*={top_coldspot['gi_star']:.2f})"
        )
...
def main():
    ...
    output_dir = settings.ANALYSIS_OUTPUT_DIR / "analyze_spatial_hotspots"
