...
def clean_lease_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    logger.info(
        f"Lease range: {df['remaining_lease_years'].min():.0f} - {df['remaining_lease_years'].max():.0f} years"
    )
...
def visualize_lease_decay(df: pd.DataFrame, output_dir: Path):
    ...
    ax.set_title(
        "HDB Resale Price Distribution by Remaining Lease Band", fontsize=14, fontweight="bold"
    )
...
def main():
    ...
    output_dir = settings.DATA_DIR / "analysis" / "lease_decay"
