...
def load_transaction_data() -> pd.DataFrame:
    ...
    path = settings.l1_dir / "housing_hdb_transaction.parquet"
    if not path.exists():
...
def run_did_analysis(
    ...
    df = df[
        (df["month"] >= policy_dt - pd.DateOffset(months=pre_months))
...
def summarize_results(result: dict, policy_date: str) -> list:
    ...
    treatment_change = (
        (result["post_treatment_treatment"] - result["pre_treatment_treatment"])
        / result["pre_treatment_treatment"]
        * 100
    )
    control_change = (
        (result["post_treatment_control"] - result["pre_treatment_control"])
        / result["pre_treatment_control"]
        * 100
    )
...
def main():
    ...
    output_dir = settings.ANALYSIS_OUTPUT_DIR / "analyze_policy_impact"
