...
def load_and_prepare_data():
    ...
    if H3_AVAILABLE:
        ...
        df["h8_cell"] = df.apply(
            lambda row: h3.latlng_to_cell(float(row["lat"]), float(row["lon"]), 8), axis=1
        )
...
def run_ols_regression(df, target_col="price_psf"):
    ...
    X_train, X_test, y_train, y_test = train_test_split(X_inv, y, test_size=0.2, random_state=42)
...
def create_feature_importance_chart(all_importance, target_col="price_psf"):
    ...
    ax.set_title(
        f"XGBoost Feature Importance for {target_col.replace('_', ' ').title()}",
        fontsize=14,
        fontweight="bold",
    )
...
def create_distance_band_premiums_chart(df):
    ...
    band_prices = (
        df.groupby(["dist_band", "property_type"], observed=True)["price_psf"].mean().reset_index()
    )
    ...
    for prop_type in band_prices.columns:
        if prop_type in colors:
            ax.plot(
                band_midpoints,
                band_prices[prop_type],
                marker=markers[prop_type],
                markersize=8,
                linewidth=2,
                label=prop_type,
    ...
    ax.set_title(
        "Price Premium by MRT Distance Band and Property Type", fontsize=14, fontweight="bold"
    )
...
def main():
    ...
    for target_col, target_name in targets.items():
        ...
        coef_df = extract_feature_coefficients(best_ols["model"], best_ols["features"], target_col)
