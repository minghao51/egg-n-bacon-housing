...
# Paths
DATA_DIR = Path("data/pipeline/04_platinum")
...
def analyze_by_flat_type(hdb, feature="school_accessibility_score"):
    ...
    if not results_df.empty:
        ...
        print(
            results_df[["flat_type", "n", "mean_price", "school_premium_10pt", "r2"]].to_string(
                index=False
            )
        )
    else:
...
def analyze_by_town(hdb, feature="school_accessibility_score", min_n=500):
    ...
    if not results_df.empty:
        ...
        print(
            results_df.head(10)[["town", "n", "mean_price", "school_premium_10pt"]].to_string(
                index=False
            )
        )
        ...
        print(
            results_df.tail(10)[["town", "n", "mean_price", "school_premium_10pt"]].to_string(
                index=False
            )
        )
    else:
...
def analyze_by_lease_remaining(hdb, feature="school_accessibility_score"):
        ...
        results.append(
            {
                ...
                "n": len(X),
                "mean_price": y.mean(),
                "mean_lease_years": X[:, 0].mean()
                if "remaining_lease_months" not in feature_cols
                else subset["lease_years"].mean(),
                "school_premium_10pt": school_coef * 10,
                "r2": r2,
            }
        )
    ...
    if not results_df.empty:
        ...
        print(
            results_df[["lease_band", "n", "mean_price", "school_premium_10pt"]].to_string(
                index=False
            )
        )
    else:
...
def analyze_by_price_tier(hdb, feature="school_accessibility_score"):
    ...
    if not results_df.empty:
        ...
        print(
            results_df[
                ["price_tier", "n", "mean_price", "price_range", "school_premium_10pt"]
...
def visualize_heterogeneous_effects(
    flat_type_results, town_results, lease_results, price_tier_results
):
    ...
    if not flat_type_results.empty:
        ...
        ax.barh(
            flat_type_results["flat_type"], flat_type_results["school_premium_10pt"], color=colors
        )
    ...
    if not town_results.empty:
        ax.scatter(
            town_results["mean_price"],
            town_results["school_premium_10pt"],
            alpha=0.6,
            s=100,
            edgecolors="black",
            linewidths=0.5,
...
def main():
    ...
    if not flat_type_results.empty:
        print("\nBy Flat Type:")
        print(
            f"  Range: ${flat_type_results['school_premium_10pt'].min():.2f} to ${flat_type_results['school_premium_10pt'].max():.2f} per 0.1 score"
        )
        most_sensitive = (
            flat_type_results.iloc[0]
            if flat_type_results["school_premium_10pt"].abs().idxmax() == flat_type_results.index[0]
            else flat_type_results.loc[flat_type_results["school_premium_10pt"].abs().idxmax()]
        )
        print(
            f"  Most sensitive: {most_sensitive['flat_type']} (${abs(most_sensitive['school_premium_10pt']):.2f})"
        )

    if not town_results.empty:
        print("\nBy Town:")
        print(
            f"  Range: ${town_results['school_premium_10pt'].min():.2f} to ${town_results['school_premium_10pt'].max():.2f} per 0.1 score"
        )
        print(
            f"  Most sensitive: {town_results.iloc[0]['town']} (${abs(town_results.iloc[0]['school_premium_10pt']):.2f})"
        )
