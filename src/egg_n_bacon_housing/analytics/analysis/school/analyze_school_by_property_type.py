...
def analyze_by_property_type(df, target_col="price_psf"):
    ...
    for property_type in property_types:
        ...
        base_features = ["floor_area_sqm", "year"]
        ...
        if len(X) < 50:
            print(
                f"  Skipping {property_type}: insufficient data after cleaning ({len(X)} records)"
            )
            continue
...
def compare_property_types(results):
    ...
    for pt, data in results.items():
        row = {"property_type": pt, "n": data["n"], "mean_price": data["mean_price"]}
    ...
    school_coef_cols = [
        col for col in comparison_df.columns if "school" in col and "_coef" not in col
    ]
    if school_coef_cols:
...
def visualize_comparison(results, comparison_df):
    ...
    if "school_accessibility_score" in comparison_df.columns:
        ...
        ax.bar(
            comparison_df["property_type"],
            comparison_df["school_accessibility_score"],
            color=colors,
        )
        ...
        ax.text(
            0.5,
            0.5,
            "School accessibility score not available",
            ha="center",
            va="center",
...
def create_interaction_model(df, target_col="price_psf"):
    ...
    if len(baseline_coef) > 0:
        ...
        if "school_x_condo" in coefs["feature"].values:
            condo_interaction = coefs.loc[
                coefs["feature"] == "school_x_condo", "coefficient"
            ].values[0]
            condo_effect = baseline_coef[0] + condo_interaction
            print(f"  Condo: School coefficient = ${condo_effect:.4f} PSF/score point")
            print(
                f"    (baseline + interaction = ${baseline_coef[0]:.4f} + ${condo_interaction:.4f})"
            )
...
def main():
    ...
    hdb_school_coverage = (
        df[df["property_type"] == "HDB"]["school_accessibility_score"].notna().sum()
        if "school_accessibility_score" in df.columns
        else 0
    )
    condo_school_coverage = (
        df[df["property_type"] == "Condominium"]["school_accessibility_score"].notna().sum()
        if "school_accessibility_score" in df.columns
        else 0
    )
    ...
    for _, row in comparison_df.iterrows():
        ...
        if "school_accessibility_score" in row and pd.notna(row["school_accessibility_score"]):
            print(
                f"    School Accessibility Coefficient: ${row['school_accessibility_score']:.4f} per score point"
            )
