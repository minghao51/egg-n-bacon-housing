...
# Output directory
OUTPUT_DIR = settings.DATA_DIR / "analysis" / "appreciation_patterns"
...
def load_appreciation_data(min_year=2017):
    ...
    path = settings.l3_dir / "housing_unified.parquet"
    ...
    logger.info(
        f"  YoY range: {df['yoy_change_pct'].min():.1f}% to {df['yoy_change_pct'].max():.1f}%"
    )
...
def analyze_mrt_impact_on_appreciation(df):
    ...
    mrt_impact = (
        df_clean.groupby("mrt_distance_bin", observed=True)
        .agg({"yoy_change_pct": ["mean", "median", "count"], "price_psf": "median"})
        .round(2)
    )
    ...
    feature_cols = ["dist_to_nearest_mrt", "floor_area_sqm", "remaining_lease_months", "price_psf"]
    ...
    logger.info(
        f"  Interpretation: Every 100m closer to MRT = {model.coef_[0] * 100:.2f}% change in YoY appreciation"
    )
    ...
    for prop_type in ["HDB", "Condominium", "EC"]:
        ...
        mrt_type = (
            df_type.groupby("mrt_distance_bin", observed=True)
            .agg({"yoy_change_pct": ["mean", "count"]})
            .round(2)
        )
...
def analyze_temporal_appreciation_dynamics(df):
    ...
    yearly_stats = (
        df.groupby(["year", "property_type"])["yoy_change_pct"]
    ...
    covid = (
        yearly_stats[yearly_stats["year"].between(2020, 2022)]
...
def create_appreciation_visualizations(
    df, prop_stats, area_stats, cluster_summary, results_df, importance_df, yearly_stats, hotspot_df
):
    ...
    ax.set_title(
        "Distribution of Appreciation Rates by Property Type", fontsize=14, fontweight="bold"
    )
    ax.legend()
...
def generate_investment_insights(df, cluster_summary, hotspot_df, importance_df):
    ...
    if prop_mean.idxmax() == "Condominium":
        insights.append(
            "✓ Condos show highest appreciation - suitable for growth-focused investors"
        )
    ...
    insights.append(
        f"✓ {len(elite)} elite hotspot areas identified with consistently high appreciation"
    )
...
def main():
    ...
    create_appreciation_visualizations(
        df,
        prop_stats,
        area_stats,
        cluster_summary,
    ...
    logger.info(
        f"  • Best model R²: {results_df['r2'].max():.4f} ({results_df.loc[results_df['r2'].idxmax(), 'model']})"
    )
