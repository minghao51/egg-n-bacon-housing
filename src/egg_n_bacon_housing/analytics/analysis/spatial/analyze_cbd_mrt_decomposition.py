...
# Output directory
OUTPUT_DIR = settings.DATA_DIR / "analysis" / "cbd_mrt_decomposition"
...
def run_hierarchical_regression(df):
    ...
    results.append(
        {"Model": "Model 1: CBD only", "R²": r2_1, "CBD Coef": model1.coef_[0], "MRT Coef": None}
    )
    ...
    # Model 3: Full model
    X3 = df_clean[
        ["dist_to_cbd_m", "dist_to_nearest_mrt", "floor_area_sqm", "remaining_lease_months"]
    ].values
...
def run_pca_analysis(df):
    ...
    loadings_df = pd.DataFrame(
        loadings, columns=["PC1", "PC2"], index=["MRT Distance", "CBD Distance"]
    )
...
def compare_regional_effects(df):
    ...
    for region in ["CCR (Core Central)", "RCR (Rest of Central)", "OCR (Outside Central)"]:
        ...
        X = df_region[
            ["dist_to_nearest_mrt", "dist_to_cbd_m", "floor_area_sqm", "remaining_lease_months"]
        ].values
...
def create_regional_effects_comparison_chart(regional_df):
    ...
    # Add overall title
    fig.suptitle(
        "Regional Effects Comparison: MRT vs CBD Accessibility",
        fontsize=15,
        fontweight="bold",
        y=1.0,
    )
    ...
    for region, color in [
        ...
        if "dist_to_cbd_km" in sample.columns:
            ...
            if len(region_sample) > 0:
                ax6.scatter(
                    region_sample["dist_to_nearest_mrt"],
...
def main():
    ...
    logger.info(
        f"  • VIF - MRT: {vif_df[vif_df['Feature'] == 'dist_to_nearest_mrt']['VIF'].values[0]:.2f}"
    )
    logger.info(
        f"  • VIF - CBD: {vif_df[vif_df['Feature'] == 'dist_to_cbd_m']['VIF'].values[0]:.2f}"
    )

    logger.info(f"\n  • Pure CBD Effect (Model 1): R² = {hierarchical_df.iloc[0]['R²']:.4f}")
    logger.info(f"  • Adding MRT (Model 2): ΔR² = {hierarchical_df.iloc[1]['ΔR²']:.4f}")
    logger.info(
        f"  • MRT Premium in OCR: ${regional_df[regional_df['Region'] == 'OCR (Outside Central)']['MRT Premium $/100m'].values[0]:.2f}/100m"
    )
