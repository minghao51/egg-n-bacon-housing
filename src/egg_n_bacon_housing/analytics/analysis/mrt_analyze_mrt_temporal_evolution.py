...
# Output directory
OUTPUT_DIR = settings.DATA_DIR / "analysis" / "mrt_temporal_evolution"
...
def plot_temporal_evolution(yearly_results, property_types=["HDB", "Condominium", "EC"]):
    ...
    ax.set_title(
        "MRT Premium Evolution by Property Type (2017-2026)", fontsize=14, fontweight="bold"
    )
    ax.legend()
...
def plot_top_areas_evolution(area_results, property_type="HDB", top_n=10):
    ...
    area_means = (
        data.groupby("planning_area")["mrt_premium_100m"].mean().sort_values(ascending=False)
    )
    ...
    ax.set_title(
        f"Top {top_n} Planning Areas: MRT Premium Evolution ({property_type})",
        fontsize=14,
        fontweight="bold",
    )
...
def main():
    ...
    area_results_condo = analyze_by_planning_area(
        df, property_type="Condominium", min_transactions=100
    )
