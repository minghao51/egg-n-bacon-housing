...
def get_analysis_output_dir(script_name: str) -> Path:
    ...
    Example:
        >>> from egg_n_bacon_housing.utils.utils import get_analysis_output_dir
        >>> from egg_n_bacon_housing.config import settings
    ...
    output_dir = settings.data_dir / "analysis" / script_name
    output_dir.mkdir(exist_ok=True, parents=True)
    return output_dir
...
def get_pipeline_output_dir(pipeline_name: str) -> Path:
    ...
    Example:
        >>> from egg_n_bacon_housing.utils.utils import get_pipeline_output_dir
        >>> output_dir = get_pipeline_output_dir('forecast_prices')
    """
    from egg_n_bacon_housing.config import settings

    output_dir = settings.data_dir / "pipelines" / pipeline_name
    output_dir.mkdir(exist_ok=True, parents=True)
    return output_dir
