...
from src.data_loader import load_unified_data, get_unified_data_summary
from src.ui_components import (
    load_css, page_header, section_header, divider,
    display_metrics_row, info_box
)
...
with col3:
    ...
    st.metric(
        "Planning Areas",
        planning_areas,
        help="Number of URA planning areas covered"
    )
    ...
    if 'property_type' in df.columns:
        ...
        with ptype_col1:
            if 'HDB' in prop_counts:
                st.metric("HDB Transactions", f"{prop_counts['HDB']:,}",
                         f"{prop_counts['HDB']/len(df)*100:.1f}%")
        with ptype_col2:
            if 'Condominium' in prop_counts:
                st.metric("Condo Transactions", f"{prop_counts['Condominium']:,}",
                         f"{prop_counts['Condominium']/len(df)*100:.1f}%")
        with ptype_col3:
            if 'EC' in prop_counts:
                st.metric("EC Transactions", f"{prop_counts['EC']:,}",
...
with col2:
    ...
    if 'rental_yield_pct' in df.columns:
        ...
        if not rental_data.empty:
            ...
            st.dataframe(
                top_rental.to_frame('Yield %').style.format('{:.2f}'),
            ...
            if 'flat_type' in rental_data.columns:
                ...
                st.dataframe(
                    top_combo.to_frame('Yield %').style.format('{:.2f}'),
                    use_container_width=True
                )
        else:
...
if 'planning_area' in df.columns:
    ...
    with pa_col1:
        st.metric("Most Active Area", f"{pa_counts.index[0]}",
                 f"{pa_counts.iloc[0]:,} transactions")

    with pa_col2:
        st.metric("Coverage", f"{df['planning_area'].nunique()} planning areas",
        ...
        if '90+ years' in lease_stats.index:
            ...
            with ld_col1:
                if '<60 years' in lease_stats.index:
                    ...
                    st.metric("<60 Year Lease Discount", f"{discount_60:.1f}%",
                             help="Properties with <60 years lease vs 90+ years")

            with ld_col2:
                if '60-70 years' in lease_stats.index:
                    discount_70 = ((baseline_median - lease_stats.loc['60-70 years', 'Median Price']) / baseline_median * 100)
                    st.metric("60-70 Year Lease Discount", f"{discount_70:.1f}%",
                             help="Properties with 60-70 years lease vs 90+ years")

            with ld_col3:
                if '70-80 years' in lease_stats.index:
                    discount_80 = ((baseline_median - lease_stats.loc['70-80 years', 'Median Price']) / baseline_median * 100)
                    st.metric("70-80 Year Lease Discount", f"{discount_80:.1f}%",
                             help="Properties with 70-80 years lease vs 90+ years")

            # Display table
            st.markdown("**Lease Band Statistics:**")
            display_stats = lease_stats.copy()
            display_stats['Median Price'] = display_stats['Median Price'].apply(lambda x: f"${x:,.0f}")
            ...
            st.info("Lease decay analysis requires properties with 90+ years remaining lease as baseline")
    else:
...
if 'mom_change_pct' in df.columns:
    ...
    if not metrics_data.empty:
        ...
        with metrics_col2:
            avg_yoy = metrics_data['yoy_change_pct'].mean() if 'yoy_change_pct' in metrics_data.columns else 0
            st.metric("Avg Yearly Growth", f"{avg_yoy:.2f}%")

        with metrics_col3:
            momentum_bullish = (metrics_data['momentum_signal'] == 'Strong Acceleration').sum() if 'momentum_signal' in metrics_data.columns else 0
        ...
        if 'transaction_date' in metrics_data.columns:
            ...
            if not recent_metrics.empty:
                ...
                with trend_col1:
                    ...
                    st.metric("Recent Monthly Growth", f"{recent_mom:.2f}%",
                             delta_color="normal")

                with trend_col2:
                    recent_vol = recent_metrics['transaction_count'].mean() if 'transaction_count' in recent_metrics.columns else 0
                    st.metric("Avg Monthly Volume", f"{recent_vol:.0f}",
                             delta_color="normal")
    else:
...
if 'planning_area' in df.columns:
    ...
    insights.append(f"{top_pa} has the highest transaction volume at {top_pa_pct:.1f}% of all transactions")
...
if amenity_distance_cols:
    ...
    if best_mrt < 500:
        insights.append(f"{ (df['dist_to_nearest_mrt'] <= 500).sum():,} properties are within 500m of an MRT station")
