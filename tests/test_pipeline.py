...
def sample_api_response():
    ...
    return {
        "result": {
            "records": [{"quarter": "2024-Q1", "value": 100}, {"quarter": "2024-Q2", "value": 200}],
            "total": 2,
            "_links": {},
        }
    }
...
class TestL0Collect:
    ...
    def test_fetch_datagovsg_dataset(self, mock_get, mock_cached_call):
        ...
        mock_response.json.return_value = {
            "result": {"records": [{"a": 1}, {"b": 2}], "total": 2, "_links": {}}
        }
        ...
        # Fetch data
        result = fetch_datagovsg_dataset("https://api.test.com/", "test_id", use_cache=False)
    ...
    def test_load_resale_flat_prices(self, mock_config, mock_read_csv):
        ...
        mock_resale_path.glob.return_value = [Mock(name="file1.csv"), Mock(name="file2.csv")]
        ...
        mock_read_csv.return_value = pd.DataFrame(
            {"month": ["2024-01"], "town": ["BISHAN"], "remaining_lease": ["61 years 04 months"]}
        )
...
class TestL1Process:
    ...
    def test_prepare_unique_addresses(self, mock_extract):
        ...
        mock_df = pd.DataFrame(
            {"NameAddress": ["Address 1", "Address 2"], "property_type": ["private", "hdb"]}
        )
        mock_extract.return_value = mock_df

        # Prepare
        result = prepare_unique_addresses(pd.DataFrame(), pd.DataFrame(), pd.DataFrame())
    ...
    def test_process_geocoded_results_empty(self, mock_save):
        ...
        result = process_geocoded_results(
            pd.DataFrame(), pd.DataFrame(), save_full=True, save_filtered=True
        )
...
class TestPipelineIntegration:
        ...
        def mock_response_func(url, *args, **kwargs):
            ...
            if "d_5785799d63a9da091f4e0b456291eeb8" in url:
                mock_resp.json.return_value = {
                    "result": {"records": [{"quarter": "2024-Q1"}], "total": 1, "_links": {}}
                }
            else:
                mock_resp.json.return_value = {"result": {"records": [], "total": 0, "_links": {}}}
            return mock_resp
    ...
    def test_full_l1_pipeline_workflow(self, mock_process, mock_prepare, mock_load, mock_geocode):
        ...
        mock_geocode.return_value = (
            pd.DataFrame({"search_result": [0, 0], "NameAddress": ["Address 1", "Address 2"]}),
            [],
        )
