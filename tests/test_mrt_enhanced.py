...
def main():
    ...
    logger.info(
        f"  Median distance to MRT: {sample_with_mrt['nearest_mrt_distance'].median():.0f}m"
    )
    ...
    for tier, count in tier_counts.items():
        logger.info(
            f"  Tier {int(tier)}: {count:,} properties ({count / len(sample_with_mrt) * 100:.1f}%)"
        )
    ...
    logger.info(
        f"  Properties near interchanges: {interchange_count:,} ({interchange_count / len(sample_with_mrt) * 100:.1f}%)"
    )
