...
def calculate_psm_tiers(df: pd.DataFrame) -> pd.DataFrame:
    """Calculate PSM-based tiers for standardized comparison.

    Args:
        df: Transaction data with price_psm column

    Returns:
        DataFrame with psm_tier column added
    """
    logger.info("\nCalculating PSM tiers...")

    df = df.copy()

    # Calculate PSM tiers within each property type
    ...
    # Apply tier assignment within each property type
    if 'property_type' in df.columns:
        df['psm_tier'] = df.groupby('property_type', group_keys=False).apply(
            lambda g: assign_psm_tier(g)
        )
    else:
        df['psm_tier'] = assign_psm_tier(df)

    # Report PSM tier distribution
    logger.info("\nPSM Tier Distribution:")
    for ptype in df['property_type'].unique():
        ptype_data = df[df['property_type'] == ptype]
        logger.info(f"\n{ptype}:")
        for tier in ['Low PSM', 'Medium PSM', 'High PSM']:
            count = (ptype_data['psm_tier'] == tier).sum()
            pct = count / len(ptype_data) * 100
            tier_psm = ptype_data[ptype_data['psm_tier'] == tier]['price_psm']
            logger.info(f"  {tier}: {count:,} ({pct:.1f}%) - Median: ${tier_psm.median():,.0f}/sqm")
...
def analyze_tier_performance(df: pd.DataFrame) -> pd.DataFrame:
    ...
    performance.columns = ['property_type', 'market_tier', 'year',
                          'median_price', 'transaction_count', 'median_psm']
