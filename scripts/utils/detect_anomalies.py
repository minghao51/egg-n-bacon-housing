...
high_missing = ["Project Name", "Street Name", "Postal District", "Property Type", "Market Segment"]
...
market_cols = ["transaction_count", "volume_3m_avg", "volume_12m_avg", "stratified_median_price"]
...
categorical_features = (
    df_recent[feature_cols].select_dtypes(include=["object", "category"]).columns.tolist()
)
...
undervalued_pred["potential_savings"] = (
    undervalued_pred["residual"] * undervalued_pred["floor_area_sqm"]
)
...
df_recent["iso_anomaly"] = df_recent["iso_anomaly_score"].apply(
...
df_recent_sample["lof_anomaly"] = df_recent_sample["lof_anomaly_score"].apply(
    lambda x: "Anomaly" if x == -1 else "Normal"
)
...
top_opportunities = df_recent[
    (df_recent["residual"] < threshold) & (df_recent["anomaly_methods"] >= 2)
].copy()
...
if len(top_opportunities) > 0:
    ...
    display_df["potential_savings_pct"] = display_df["potential_savings_pct"].apply(
        lambda x: f"{x:.1f}%"
    )
    display_df["price_psm"] = display_df["price_psm"].apply(lambda x: f"${x:,.0f}")
    display_df["predicted_price_psm"] = display_df["predicted_price_psm"].apply(
        lambda x: f"${x:,.0f}"
    )
...
axes[0, 0].axvline(
    ...
    linestyle="--",
    linewidth=2,
    label=f"Undervalued Threshold (${threshold:.0f})",
)
...
axes[0, 1].scatter(
    undervalued_pred["predicted_price_psm"],
    undervalued_pred["price_psm"],
    alpha=0.6,
    s=20,
    color="red",
    label="Undervalued",
...
axes[1, 1].set_title(
    "Towns with Most Undervalued Properties (Average)", fontsize=14, fontweight="bold"
)
...
if len(top_opportunities) > 0:
    ...
    ax.set_yticklabels(
        [f"{row['town']} - {row['flat_type']}" for _, row in top_20.iterrows()], fontsize=9
    )
    ax.set_xlabel("Potential Savings ($)", fontsize=12, fontweight="bold")
    ax.set_title(
        "Top 20 Undervalued Properties by Potential Savings", fontsize=14, fontweight="bold"
    )
...
all_anomalies[export_cols].to_csv(OUTPUT_DIR / "all_anomalies.csv", index=False)
print(f"  Saved: all_anomalies.csv ({len(all_anomalies)} properties)")

# Export top opportunities
if len(top_opportunities) > 0:
    top_opportunities[export_cols].to_csv(OUTPUT_DIR / "top_opportunities.csv", index=False)
