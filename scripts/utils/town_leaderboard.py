...
for town in town_metrics["town"]:
    if town in town_growth_pivot.index:
        ...
        if len(years) >= 2:
            ...
            last_year = max(years)
            if pd.notna(town_growth_pivot.loc[town, first_year]) and pd.notna(
                town_growth_pivot.loc[town, last_year]
            ):
...
if "price_psm_median" in town_metrics.columns:
    ...
    town_metrics["affordability_score"] = 1 - (
        (town_metrics["price_psm_median"] - town_metrics["price_psm_median"].min())
...
if "price_psm_count" in town_metrics.columns:
    town_metrics["liquidity_score"] = (
        town_metrics["price_psm_count"] - town_metrics["price_psm_count"].min()
...
if "rental_yield_pct_mean" in town_metrics.columns:
    # Normalize yield
    town_metrics["rental_yield_norm"] = (
        town_metrics["rental_yield_pct_mean"] - town_metrics["rental_yield_pct_mean"].min()
...
if "price_growth_yoy" in town_metrics.columns:
    # Normalize growth
    town_metrics["price_growth_norm"] = (
        town_metrics["price_growth_yoy"] - town_metrics["price_growth_yoy"].min()
...
if "mom_change_pct_mean" in town_metrics.columns:
    ...
    if mom_range > 0:
        town_metrics["mom_change_norm"] = (
            town_metrics["mom_change_pct_mean"] - mom_min
        ) / mom_range
...
print("-" * 80)
top_investor = town_metrics_sorted.head(10)[
    ["town", "investor_score", "rental_yield_pct_mean", "price_growth_yoy", "market_activity_score"]
].copy()
top_investor["investor_score"] = top_investor["investor_score"].apply(lambda x: f"{x:.3f}")
top_investor["rental_yield_pct_mean"] = top_investor["rental_yield_pct_mean"].apply(
    lambda x: f"{x:.2f}%"
)
top_investor["price_growth_yoy"] = top_investor["price_growth_yoy"].apply(lambda x: f"{x:.2f}%")
top_investor["market_activity_score"] = top_investor["market_activity_score"].apply(
    lambda x: f"{x:.3f}"
)
...
print("-" * 80)
top_value = (
    town_metrics.sort_values("value_score", ascending=False)
    .head(10)[["town", "value_score", "price_psm_median", "rental_yield_pct_mean"]]
    .copy()
)
...
print("-" * 80)
top_momentum = (
    town_metrics.sort_values("momentum_score", ascending=False)
    .head(10)[["town", "momentum_score", "mom_change_pct_mean", "price_growth_yoy"]]
    .copy()
)
top_momentum["momentum_score"] = top_momentum["momentum_score"].apply(lambda x: f"{x:.3f}")
top_momentum["mom_change_pct_mean"] = top_momentum["mom_change_pct_mean"].apply(
    lambda x: f"{x:.2f}%"
)
...
print("-" * 80)
top_affordable = (
    town_metrics.sort_values("price_psm_median")
    .head(10)[["town", "price_psm_median", "floor_area_sqm_mean", "rental_yield_pct_mean"]]
    .copy()
)
top_affordable["price_psm_median"] = top_affordable["price_psm_median"].apply(
    lambda x: f"${x:,.0f}"
)
top_affordable["floor_area_sqm_mean"] = top_affordable["floor_area_sqm_mean"].apply(
    lambda x: f"{x:,.0f} sqm"
)
top_affordable["rental_yield_pct_mean"] = top_affordable["rental_yield_pct_mean"].apply(
    lambda x: f"{x:.2f}%"
)
...
print("-" * 80)
top_liquid = (
    town_metrics.sort_values("price_psm_count", ascending=False)
    .head(10)[["town", "price_psm_count", "transaction_count_mean", "volume_12m_avg_mean"]]
    .copy()
)
top_liquid["price_psm_count"] = top_liquid["price_psm_count"].apply(lambda x: f"{x:,.0f}")
top_liquid["transaction_count_mean"] = top_liquid["transaction_count_mean"].apply(
    lambda x: f"{x:,.0f}"
)
...
scatter = axes[0].scatter(
    town_metrics["price_psm_median"],
    town_metrics["rental_yield_pct_mean"],
...
axes[0].set_title(
    "Price vs Yield (Size = Transaction Count, Color = Investor Score)",
    fontsize=13,
    fontweight="bold",
)
...
axes[1].set_title(
    "Growth vs Yield (Size = Transaction Count, Color = Momentum Score)",
    fontsize=13,
    fontweight="bold",
)
...
town_metrics[export_cols].to_csv(OUTPUT_DIR / "town_leaderboard.csv", index=False)
...
leaderboard_summary["overall_rank"] = (
    town_metrics["investor_rank"] + town_metrics["value_rank"] + town_metrics["momentum_rank"]
) / 3
leaderboard_summary = leaderboard_summary.sort_values("overall_rank")
leaderboard_summary.to_csv(OUTPUT_DIR / "town_leaderboard_summary.csv", index=False)
...
print(
    f"  • Most affordable: {town_metrics.loc[town_metrics['affordability_score'].idxmax(), 'town']}"
)
