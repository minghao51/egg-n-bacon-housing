...
def validate_crosswalk(hdb_df: pd.DataFrame, condo_df: pd.DataFrame) -> dict:
    ...
    validation_results = {
        ...
        "unique_planning_areas": pd.concat(
            [hdb_df["planning_area"], condo_df["planning_area"]]
        ).nunique(),
        "composite_mappings": len(hdb_df[hdb_df["mapping_type"] == "composite"])
        + len(condo_df[condo_df["mapping_type"] == "composite"]),
        "verified_mappings": len(hdb_df[hdb_df["validation_status"] == "verified"])
        + len(condo_df[condo_df["validation_status"] == "verified"]),
        "status": "PASS",
    }
    ...
    if validation_results["composite_mappings"] > 0:
        validation_results["warnings"] = (
            f"{validation_results['composite_mappings']} composite mappings require aggregation"
        )
...
def main():
    ...
    print(
        f"   Created {len(hdb_crosswalk)} mappings for {hdb_crosswalk['source_value'].nunique()} HDB towns"
    )

    print("\n2. Creating postal district → planning area mapping...")
    condo_crosswalk = create_postal_district_to_planning_area()
    print(
        f"   Created {len(condo_crosswalk)} mappings for {condo_crosswalk['source_value'].nunique()} postal districts"
    )
