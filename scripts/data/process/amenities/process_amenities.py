...
def extract_html_name(html_string: str, tag_name: str) -> str:
...
def parse_datagov_geojson(filepath, amenity_type, name_field="Name"):
    ...
    if name_field in gdf.columns:
        ...
        if amenity_type == "preschool":
            ...
            gdf["name"] = [
                extract_html_name(str(desc), "CENTRE_NAME")
                if "CENTRE_NAME" in str(desc)
                else extract_html_name(str(desc), name_field)
    ...
    if "Description" in gdf.columns and name_field not in gdf.columns:
        ...
        logger.info(
            f"  HTML parsing: {after_count}/{before_count} names extracted ({extraction_rate:.1f}%)"
        )
    else:
        df = df[df["name"] != ""]
...
def main():
    ...
    l1_data_path = data_base_path / "L1"
    l1_data_path.mkdir(parents=True, exist_ok=True)
    ...
    if pc_path.exists():
        ...
        pc_gdf.to_file(l1_data_path / "park_connector.geojson", driver="GeoJSON")
        logger.info("✅ Saved park_connector.geojson")
