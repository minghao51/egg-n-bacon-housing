...
mrt_pre = temporal_df[
    (temporal_df["period"] == "pre_covid") & (temporal_df["feature"] == "dist_to_nearest_mrt")
]["importance"].values
mrt_covid = temporal_df[
    (temporal_df["period"] == "covid") & (temporal_df["feature"] == "dist_to_nearest_mrt")
]["importance"].values
mrt_post = temporal_df[
    (temporal_df["period"] == "post_covid") & (temporal_df["feature"] == "dist_to_nearest_mrt")
]["importance"].values
...
print("\n=== KEY FINDINGS ===")
print(
    f"MRT importance change (Pre-COVID to Post-COVID): {summary.get('mrt_importance_change_pct', 'N/A'):.1f}%"
)
print(
    f"MRT proximity premium (closest vs farthest band): {summary.get('mrt_proximity_premium_pct', 'N/A'):.1f}%"
)
