...
def aggregate_by_timeperiod(df: pd.DataFrame, granularity: str = "monthly") -> pd.DataFrame:
    """
    ...
    # Aggregate metrics
    agg_funcs = {price_col: ["median", "mean", "count", "std"]}
...
def create_trend_line_chart(
    ...
    fig.add_trace(
        go.Scatter(
            ...
            marker=dict(size=6),
            hovertemplate=(f"<b>%{{x}}</b><br>{metric}: %{{y:,.0f}}<br><extra></extra>"),
        )
    )
...
def create_comparison_boxplot(
    df: pd.DataFrame, group_column: str = "town", price_column: str | None = None
) -> go.Figure:
    """
    ...
    median_prices = (
        df_filtered.groupby(group_column)[price_column].median().sort_values(ascending=True)
    )
    df_filtered[group_column] = pd.Categorical(
        df_filtered[group_column], categories=median_prices.index, ordered=True
    )
...
def create_volume_bar_chart(
    df: pd.DataFrame, period_column: str = "period_str", count_column: str = "transaction_count"
) -> go.Figure:
    """
    ...
    fig.add_trace(
        go.Bar(
            ...
            y=df[count_column],
            marker_color="#3498DB",
            hovertemplate=("<b>%{x}</b><br>Transactions: %{y:,.0f}<br><extra></extra>"),
        )
    )
...
def create_growth_rate_chart(
    df: pd.DataFrame, period_column: str = "period_str", growth_column: str = "price_growth_pct"
) -> go.Figure:
    """
    ...
    fig.add_trace(
        go.Bar(
            ...
            y=df[growth_column],
            marker_color=colors,
            hovertemplate=("<b>%{x}</b><br>Growth: %{y:.2f}%<br><extra></extra>"),
        )
    )
...
def create_correlation_heatmap(df: pd.DataFrame, columns: list[str] | None = None) -> go.Figure:
    """
    ...
    fig = go.Figure(
        data=go.Heatmap(
            ...
            textfont={"size": 10},
            hovertemplate=(
                "<b>%{x}</b> vs <b>%{y}</b><br>Correlation: %{z:.2f}<br><extra></extra>"
            ),
        )
    )
...
def display_metrics_cards(df: pd.DataFrame, columns: int = 4):
    """
...
def aggregate_investment_metrics(
    transactions_df: pd.DataFrame, rental_yield_df: pd.DataFrame, property_type: str
) -> pd.DataFrame:
    ...
    agg = (
        df.groupby(["planning_area", "property_type"])
        .agg(
            {
...
def create_investment_quadrant_chart(
    ...
    fig.add_shape(
        ...
        x1=x_max + x_pad,
        y1=y_max + y_pad,
        fillcolor=quadrant_colors["best"],
        line_width=0,
    )
    ...
    fig.add_shape(
        ...
        x1=x_max + x_pad,
        y1=y_median,
        fillcolor=quadrant_colors["cash_flow"],
        line_width=0,
    )
    ...
    fig.add_shape(
        ...
        x1=x_median,
        y1=y_max + y_pad,
        fillcolor=quadrant_colors["growth"],
        line_width=0,
    )
    ...
    fig.add_shape(
        ...
        x1=x_median,
        y1=y_median,
        fillcolor=quadrant_colors["avoid"],
        line_width=0,
    )
    ...
    df["hover_text"] = (
        ...
        + "<br>"
        + "Rental Yield: "
        + df[x_col].apply(lambda x: f"{x:.2f}%")
        + "<br>"
        + "Appreciation: "
        + df[y_col].apply(lambda x: f"{x:.2f}%")
        + "<br>"
        + "Transactions: "
        + df[size_col].apply(lambda x: f"{x:,}")
        + "<br>"
        + "Median Price: $"
        + df["median_price"].apply(lambda x: f"{x:,.0f}")
    )
