...
def prepare_features(df, target_col="price_psf"):
    ...
    categorical_features = (
        df_prep[feature_cols].select_dtypes(include=["object", "category"]).columns.tolist()
    )
...
def analyze_shap_values(pipeline, X_train, feature_names, model_name="XGBoost"):
    ...
    numeric_feature_names = (
        preprocessor.named_transformers_["num"].named_steps["scaler"].feature_names_in_
    )
    ...
    shap_df = pd.DataFrame({"feature": all_feature_names, "shap_value": mean_shap}).sort_values(
...
def main():
    ...
    # Configuration
    USE_TEMPORAL_SPLIT = (
        False  # Set True for temporal split (pre-2020 vs 2020+), False for random 80/20 split
    )
    EXTRACT_FEATURE_IMPORTANCE = True  # Extract and save feature importance from best model

    print("\nConfiguration:")
    print(
        f"  Split Method: {'Temporal (pre-2020 vs 2020+)' if USE_TEMPORAL_SPLIT else 'Random (80/20)'}"
    )
    ...
    for target_col, target_name in targets.items():
        ...
        # Prepare features
        df_prep, numeric_features, categorical_features, feature_cols = prepare_features(
            df, target_col
        )
        ...
        if USE_TEMPORAL_SPLIT and "year" in X.columns:
            ...
            X_train, X_test, y_train, y_test = train_test_split(
                X.drop(columns=["year"], errors="ignore"), y, test_size=0.2, random_state=42
            )
        ...
        for model_type, model_display_name in models_to_test:
            try:
                ...
                pipeline, metrics = train_and_evaluate(
                    X_train, X_test, y_train, y_test, pipeline, model_display_name
                )

                metrics["target"] = target_col
                metrics["target_name"] = target_name
                all_results.append(metrics)
                target_results[model_type] = {"pipeline": pipeline, "metrics": metrics}
        ...
        if "xgboost" in target_results and SHAP_AVAILABLE:
            try:
                shap_df, explainer, shap_values, X_processed, feature_names = analyze_shap_values(
                    target_results["xgboost"]["pipeline"], X_train, feature_cols, "XGBoost"
                )
        ...
        if EXTRACT_FEATURE_IMPORTANCE:
            ...
            if "random_forest" in target_results:
                try:
                    ...
                    importance_path = (
                        OUTPUT_DIR / f"feature_importance_{target_col}_random_forest.csv"
                    )
    ...
    if len(results_df) > 0:
        ...
        print(
            results_df[["model", "target_name", "test_r2", "test_mae", "test_rmse"]].to_string(
                index=False
            )
        )
    else:
