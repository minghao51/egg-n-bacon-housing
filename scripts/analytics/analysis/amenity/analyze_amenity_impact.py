...
def get_amenity_features():
    ...
    return {
        ...
        'amenities': [
            'dist_to_nearest_hawker', 'dist_to_nearest_supermarket',
            'dist_to_nearest_park', 'dist_to_nearest_preschool',
...
def run_temporal_analysis(df):
    ...
    periods = {
        'pre_covid': (2015, 2019),
        'covid': (2020, 2022),
        'post_covid': (2023, 2025)
    }

    amenity_features = (
        get_amenity_features()['mrt_distance'] +
        get_amenity_features()['amenities']
    )
    ...
    results_df.to_csv(OUTPUT_DIR / 'temporal_comparison.csv', index=False)
    logger.info(f"\nSaved: {OUTPUT_DIR / 'temporal_comparison.csv'}")
...
def run_within_town_analysis(df):
    ...
    for town in sorted(towns):
        ...
        mrt_importance = importance_df[importance_df['feature'] == 'dist_to_nearest_mrt']['importance'].values[0]
    ...
    logger.info("\nTop 10 towns by MRT distance sensitivity:")
    for _, row in results_df.head(10).iterrows():
        logger.info(f"  {row['town']}: importance={row['mrt_distance_importance']:.4f}, n={row['n_transactions']:,}")

    results_df.to_csv(OUTPUT_DIR / 'within_town_effects.csv', index=False)
    logger.info(f"\nSaved: {OUTPUT_DIR / 'within_town_effects.csv'}")
...
def run_grid_analysis(df):
    ...
    for i, grid_id in enumerate(grids):
        ...
        try:
            ...
            mrt_dist_importance = importance_df[
                importance_df['feature'] == 'dist_to_nearest_mrt'
            ]['importance'].values[0]
    ...
    logger.info(f"  Mean mrt_within_500m importance: {results_df['mrt_within_500m_importance'].mean():.4f}")
    logger.info(f"  Median mrt_within_500m importance: {results_df['mrt_within_500m_importance'].median():.4f}")
...
def run_mrt_distance_stratification(df):
    ...
    for _, row in band_stats.iterrows():
        if row['price_psm_count'] > 0:
            logger.info(f"  {row['dist_band']}: n={row['price_psm_count']:,}, "
...
def generate_summary_stats(temporal_df, within_town_df, grid_df, dist_strat_df):
    ...
    logger.info(f"\nSaved: {OUTPUT_DIR / 'summary_stats.csv'}")

    logger.info("\n=== KEY FINDINGS ===")
    if 'mrt_importance_change_pct' in summary:
        logger.info(f"MRT importance change (Pre-COVID to Post-COVID): {summary['mrt_importance_change_pct']:.1f}%")
    if 'mrt_proximity_premium_pct' in summary:
        logger.info(f"MRT proximity premium (closest vs farthest band): {summary['mrt_proximity_premium_pct']:.1f}%")
