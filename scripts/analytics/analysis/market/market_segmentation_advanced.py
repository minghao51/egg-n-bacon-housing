...
ax1.axvline(
    ...
    linestyle="--",
    linewidth=2,
    label=f"Elbow at K={optimal_k_inertia}",
)
...
plt.xlabel(
    f"PC1 ({pca.explained_variance_ratio_[0] * 100:.1f}% variance)", fontsize=12, fontweight="bold"
)
plt.ylabel(
    f"PC2 ({pca.explained_variance_ratio_[1] * 100:.1f}% variance)", fontsize=12, fontweight="bold"
)
...
cluster_profiles["segment_name"] = cluster_profiles.index.map(
...
for cluster_id in sorted(df_cluster["cluster_kmeans"].unique()):
    ...
    print(
        f"  Size: {len(cluster_data):,} properties ({len(cluster_data) / len(df_cluster) * 100:.1f}%)"
    )

    print("\n  Key Characteristics:")
    print(
        f"    • Average Price: ${cluster_data['price_psm'].mean():,.0f}/psm (±${cluster_data['price_psm'].std():.0f})"
    )
    print(
        f"    • Average Size: {cluster_data['floor_area_sqm'].mean():.0f} sqm (±{cluster_data['floor_area_sqm'].std():.0f})"
    )

    if "rental_yield_pct" in cluster_data.columns:
        print(
            f"    • Average Yield: {cluster_data['rental_yield_pct'].mean():.2f}% (±{cluster_data['rental_yield_pct'].std():.2f}%)"
        )

    if "yoy_change_pct" in cluster_data.columns:
        print(
            f"    • YoY Appreciation: {cluster_data['yoy_change_pct'].mean():.2f}% (±{cluster_data['yoy_change_pct'].std():.2f}%)"
        )

    if "mom_change_pct" in cluster_data.columns:
        print(
            f"    • MoM Change: {cluster_data['mom_change_pct'].mean():.2f}% (±{cluster_data['mom_change_pct'].std():.2f}%)"
        )

    print(
        f"    • Trading Activity: {cluster_data['transaction_count'].mean():.0f} transactions (±{cluster_data['transaction_count'].std():.0f})"
    )
...
for cluster_id in sorted(df_cluster["cluster_kmeans"].unique()):
    ...
    avg_yield = (
        cluster_data["rental_yield_pct"].mean() if "rental_yield_pct" in cluster_data.columns else 0
    )
    avg_growth = (
        cluster_data["yoy_change_pct"].mean() if "yoy_change_pct" in cluster_data.columns else 0
    )
    ...
    if avg_yield > 6 and avg_growth > 10:
        ...
        strategy = (
            "💰 **YIELD PLAY** - High rental yield. Focus on cash flow rather than appreciation."
        )
    elif avg_growth > 15:
        ...
        strategy = (
            "🔄 **BALANCED APPROACH** - Moderate yield and growth. Balanced investment strategy."
        )
...
if available_metrics:
    ...
    for i, metric in enumerate(available_metrics):
        ...
        cluster_means = (
            df_cluster.groupby("cluster_kmeans")[metric].mean().sort_values(ascending=False)
        )

        # Plot
        cluster_means.plot(kind="bar", ax=axes[i], color="steelblue")
        axes[i].set_title(
            f"{metric.replace('_', ' ').title()} by Segment", fontsize=12, fontweight="bold"
        )
...
df_cluster.to_csv(OUTPUT_DIR / "property_segments.csv", index=False)
