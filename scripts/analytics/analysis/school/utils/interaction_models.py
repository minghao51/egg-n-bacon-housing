...
class SegmentationAnalyzer:
    ...
    def run_segmented_models(
        self, df: pd.DataFrame, feature_cols: list, target_col: str = "price_psf"
    ):
        """
        ...
        for segment in df["market_segment"].unique():
            ...
            if len(segment_df) < 500:
                logger.warning(
                    f"  Skipping {segment}: insufficient data ({len(segment_df)} records)"
                )
                continue
            ...
            for i, feat in enumerate(feature_cols):
                if "school" in feat.lower():
                    school_coefs.append(
                        {"segment": segment, "feature": feat, "coefficient": model.coef_[i]}
                    )
    ...
    def run_interaction_model(
        self, df: pd.DataFrame, base_features: list, target_col: str = "price_psf"
    ):
        """
    ...
    def compare_r2_across_segments(self):
        ...
        for result in self.segment_results["segmented"]:
            r2_data.append(
                {"segment": result["segment"], "r2": result["r2"], "n_test": result["n_test"]}
            )
