...
def load_or_create_data(
    property_type: str = "all", start_date: str = "2021-01-01", min_transactions: int = 50
) -> pd.DataFrame:
...
def create_synthetic_data(
    property_types: list, start_date: str, min_transactions: int
) -> pd.DataFrame:
    ...
    for pt in property_types:
        ...
        for i in range(n):
            location = list(location_premiums.values())[
                np.random.randint(0, len(location_premiums))
            ]
            ...
            base_appreciation = np.random.uniform(
                config["base_appreciation"][0], config["base_appreciation"][1]
            )
            appreciation = (
                base_appreciation * (1 + location["premium"]) + spatial_autocorrelation * 0.1
            )
            ...
            records.append(
                {
                    ...
                    "property_type": pt,
                    "transaction_date": pd.to_datetime(start_date)
                    + pd.Timedelta(days=np.random.randint(0, 1825)),
                    "lat": lat,
                    "lon": lon,
                    "price_psf": np.random.uniform(
                        config["base_price_psf"][0], config["base_price_psf"][1]
                    ),
...
def aggregate_to_h3(
    df: pd.DataFrame, resolution: int = 8, min_transactions: int = 50
) -> pd.DataFrame:
    ...
    if not H3_AVAILABLE:
        ...
        df["h3_index"] = (
            pd.cut(df["lat"], bins=20).astype(str) + "_" + pd.cut(df["lon"], bins=20).astype(str)
        )
    else:
    ...
    aggregated.columns = [
        ...
        "appreciation_std",
        "n_transactions",
        "appreciation_3m_mean",
        "appreciation_5y_mean",
        "rent_yoy_mean",
        "rent_yield_mean",
        "transaction_volume",
        "price_to_income",
    ...
    logger.info(
        f"Aggregated to {len(aggregated)} H3 cells (min {min_transactions} transactions each)"
    )
    return aggregated
...
def create_comprehensive_clusters(
    lisa_df: pd.DataFrame, temporal_clusters: np.ndarray, fundamental_clusters: np.ndarray
) -> pd.DataFrame:
    ...
    logger.info(
        f"Comprehensive clusters: {result['comprehensive_cluster'].value_counts().to_dict()}"
    )
    return result


def save_results(
    moran_results: dict, lisa_df: pd.DataFrame, comprehensive_df: pd.DataFrame, output_dir: Path
):
    ...
    comprehensive_output = comprehensive_df[
        [
            "h3_index",
    ...
    cluster_summary = (
        comprehensive_df.groupby("comprehensive_cluster")
        .agg(
            {
...
def generate_findings(
    moran_result: dict, lisa_df: pd.DataFrame, comprehensive_df: pd.DataFrame
) -> list:
...
def main():
    ...
    parser.add_argument(
        "--property-type", type=str, default="all", choices=["hdb", "condo", "ec", "all"]
    )
    ...
    comprehensive_df = create_comprehensive_clusters(
        lisa_df, temporal_clusters, fundamental_clusters
    )

    output_dir = (
        Path(args.output_dir) if args.output_dir else Path("data/analytics/spatial_autocorrelation")
    )
    output_dir.mkdir(parents=True, exist_ok=True)
