...
rental_yield_by_area = rental_yield_by_area[
    rental_yield_by_area["count"] >= 50
]  # Minimum sample size
...
axes[1, 0].axvline(
    x=combined_scores["investment_score"].mean(), color="red", linestyle="--", label="Mean"
)
axes[1, 0].legend()
...
print("\n" + "-" * 120)
print(
    f"{'Planning Area':<25} {'Inv Score':<10} {'CAGR %':<10} {'Yield %':<10} {'YoY %':<10} {'Volatility':<12} {'Tier'}"
)
print("-" * 120)

for idx, row in final_recommendations.head(20).iterrows():
    print(
        f"{idx:<25} {row['investment_score']:>6.1f}     {row['cagr_pct']:>6.2f}%    {row['mean_yield']:>6.2f}%    "
...
print(
    f"- Date range: {hdb['transaction_date'].min().strftime('%Y')} to {hdb['transaction_date'].max().strftime('%Y')}"
)
