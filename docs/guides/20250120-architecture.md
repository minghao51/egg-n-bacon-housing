...
As of 2026-01-23, the L3 pipeline generates precomputed summary tables to optimize Streamlit app performance:

| Table | Rows | Description |
|-------|------|-------------|
| `market_summary.parquet` | 36 | Aggregated stats by property_type, period_5yr, market_tier_period |
| `tier_thresholds_evolution.parquet` | 36 | Tier price thresholds by period and property_type |
| `planning_area_metrics.parquet` | 41 | Metrics by planning_area with amenity distances and period breakdowns |
| `lease_decay_stats.parquet` | 5 | Price statistics by remaining lease band |
| `rental_yield_top_combos.parquet` | 114 | Town/flat_type combinations ranked by rental yield |

**Benefits**:
- Eliminates dynamic aggregation on large datasets
- Faster page load times in Streamlit apps
- Consistent metrics across all visualizations
- Cached via `@st.cache_data` in `src/data_loader.py`
...
### 1b. Precomputed Table Loaders (`src/data_loader.py`)

**Purpose**: Streamlit-optimized loaders with caching for precomputed summary tables

**Key Functions**:
- `load_market_summary()` - Market summary by property_type/period/tier
- `load_tier_thresholds()` - Tier thresholds evolution
- `load_planning_area_metrics()` - Planning area aggregated metrics
- `load_lease_decay_stats()` - Lease decay statistics
- `load_rental_yield_top_combos()` - Top rental yield combinations

**Features**:
- All functions use `@st.cache_data(ttl=3600)` for performance
- Automatic fallback for missing tables
- Precomputed during L3 pipeline execution
