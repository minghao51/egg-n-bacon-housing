...
To change the map style, edit `src/map_utils.py`:
...
Edit `src/data_loader.py` to change data locations:
...
3. **New Metric**: Add to `src/data_loader.py` aggregation functions
4. **New Visualization**: Add to `src/map_utils.py` or create new module
...
Test data loading:
```bash
uv run python -c "from src.data_loader import load_combined_data; df = load_combined_data(); print(df.head())"
```

Test map utilities:
```bash
uv run python -c "from src.map_utils import create_base_map; fig = create_base_map(); print('Map created successfully')"
```
