...
### Code
```
src/metrics.py                                    # Calculation functions
...
Edit `src/metrics.py` in `assign_price_strata()`:
```python
...
### Code
- Metrics functions: `src/metrics.py` (well-documented)
...
```python
from src.metrics import validate_metrics
df = pd.read_parquet('data/parquets/L3/metrics_monthly.parquet')
