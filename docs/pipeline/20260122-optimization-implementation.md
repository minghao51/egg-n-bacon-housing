...
### 3. Parallel Geocoding (`src/geocoding.py`)
...
### 4. Parquet Optimization (`src/data_helpers.py`)
...
### 5. Pipeline Extraction (`src/pipeline/L0_collect.py`)
...
```python
from src.geocoding import setup_onemap_headers, batch_geocode_addresses
...
```python
from src.geocoding import batch_geocode_addresses
...
1. **Extract L1 Processing** - Extract URA transaction processing to `src/pipeline/L1_process.py`
2. **Add Tests** - Create unit tests for new modules
3. **Update Notebooks** - Simplify notebooks to use extracted functions
4. **Monitor Cache** - Check cache size periodically: `from src.cache import get_cache_stats; print(get_cache_stats())`
5. **Clear Cache** - When data changes: `from src.cache import clear_cache; clear_cache()`
...
- ✅ `src/config.py` - Added cache, geocoding, parquet settings
- ✅ `src/cache.py` - NEW: Caching layer
- ✅ `src/data_helpers.py` - Parquet optimization (partitioning, compression)
- ✅ `src/geocoding.py` - Parallel geocoding functions
- ✅ `src/pipeline/L0_collect.py` - NEW: Extracted L0 logic
