...
> = {
  HH: {
    icon: "🔥",
    label: "Hotspot",
    description: "High prices surrounded by high prices",
    className:
  ...
  LL: {
    icon: "❄️",
    label: "Coldspot",
    description: "Low prices surrounded by low prices",
    className:
      "text-blue-700 bg-blue-50 dark:bg-blue-900/20 border-blue-200 dark:border-blue-800",
  },
  HL: {
    icon: "⚡",
    label: "High-Low Pioneer",
    description: "High prices in lower-priced area",
    className:
      "text-orange-700 bg-orange-50 dark:bg-orange-900/20 border-orange-200 dark:border-orange-800",
  },
  LH: {
    icon: "🔄",
    label: "Low-High Transition",
    description: "Low prices near higher-priced areas",
    className:
      "text-purple-700 bg-purple-50 dark:bg-purple-900/20 border-purple-200 dark:border-purple-800",
  },
};
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="flex items-center gap-2 mt-3">
          <span
            className={`px-3 py-1 text-sm font-medium rounded-full ${riskColor}`}
          >
            {segment.characteristics.riskLevel.replace("_", " ").toUpperCase()}{" "}
            RISK
          </span>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <StatCard
          label="Market Share"
          value={`${(segment.metrics.marketShare * 100).toFixed(1)}%`}
        />
      </div>

      {/* Hotspot Information */}
      {segment.spatialClassification &&
        HOTSPOT_INFO[segment.spatialClassification] && (
          <div
            className={`border p-4 rounded-lg ${HOTSPOT_INFO[segment.spatialClassification].className}`}
          >
            <div className="flex items-center gap-2 mb-2">
              <span className="text-2xl">
                {HOTSPOT_INFO[segment.spatialClassification].icon}
              </span>
              <div>
                <h3 className="font-semibold">
                  {HOTSPOT_INFO[segment.spatialClassification].label} Area
                </h3>
                <p className="text-sm opacity-80">
                  {HOTSPOT_INFO[segment.spatialClassification].description}
                </p>
              </div>
            </div>
            {segment.persistenceProbability > 0 && (
              <p className="text-sm mt-2">
                <strong>Persistence Probability:</strong>{" "}
                {(segment.persistenceProbability * 100).toFixed(0)}%
                {segment.spatialClassification === "HH" &&
                  " - Hotspots tend to remain hot year-over-year"}
                {segment.spatialClassification === "LL" &&
                  " - Coldspots show persistent price discounts"}
              </p>
            )}
          </div>
        )}
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3 text-sm">
          <div>
            ...
            <span className="font-medium">
              {segment.characteristics.priceTier}
            </span>
          </div>
          <div>
          ...
          <div>
            ...
            <span className="font-medium">
              {segment.characteristics.appreciationPotential}
            </span>
          </div>
          <div>
