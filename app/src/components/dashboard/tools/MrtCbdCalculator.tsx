...
export default function MrtCbdCalculator({
  data,
  persona,
}: MrtCbdCalculatorProps) {
  ...
  const basePremium =
    data.property_type_multipliers[propertyType]?.premium_per_100m || 5;
  ...
  const cbdPerKm =
    data.property_type_multipliers[propertyType]?.cbd_per_km || 15000;
      ...
      <div className="space-y-6">
        ...
        <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
          ...
          <ul className="text-sm text-blue-800 dark:text-blue-200 space-y-1">
            <li>
              • MRT premium: ${premiumPer100m}/100m for {propertyType}
            </li>
            <li>
              • CBD distance explains 22.6% of price variation vs 0.78% for MRT
            </li>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div
            className={`border rounded-lg p-4 ${netEffect >= 0 ? "bg-green-50 dark:bg-green-900/20 border-green-200 dark:border-green-800" : "bg-orange-50 dark:bg-orange-900/20 border-orange-200 dark:border-orange-800"}`}
          >
            <div
              className={`text-sm font-medium ${netEffect >= 0 ? "text-green-600 dark:text-green-400" : "text-orange-600 dark:text-orange-400"}`}
            >
              Net Effect
            </div>
            <div
              className={`text-2xl font-bold ${netEffect >= 0 ? "text-green-700 dark:text-green-300" : "text-orange-700 dark:text-orange-300"}`}
            >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div>
            <h4 className="font-medium mb-3">
              Top vs Bottom Towns (MRT Multiplier)
            </h4>
            <ClientChart style={{ height: 200 }}>
              <ResponsiveContainer width="100%" height="100%">
                <BarChart
                  data={[...topTowns, ...bottomTowns]}
                  layout="vertical"
                >
                  <CartesianGrid strokeDasharray="3 3" />
                  <XAxis type="number" />
                  ...
                  <Bar
                    dataKey="multiplier"
                    fill={(entry: any) =>
                      entry.multiplier >= 1 ? "#10b981" : "#ef4444"
                    }
                  />
                </BarChart>
              </ResponsiveContainer>
