...
export default function LeaseDecayAnalyzer({
  data,
  persona,
}: LeaseDecayAnalyzerProps) {
  ...
  const currentBand =
    data.bands.find(
      (band) =>
        remainingLease >= band.min_lease_years &&
        remainingLease <= band.max_lease_years,
    ) || data.bands[0];
  ...
  const riskZoneConfig = {
    ...
    "approaching-cliff": {
      color: "orange",
      label: "🟠 Approaching Cliff",
      bg: "bg-orange-50 dark:bg-orange-900/20",
      border: "border-orange-200 dark:border-orange-800",
    },
    cliff: {
      color: "red",
      label: "🔴 Maturity Cliff",
  ...
  const riskConfig =
    riskZoneConfig[currentBand.risk_zone as keyof typeof riskZoneConfig] ||
    riskZoneConfig.safe;
  ...
  return (
    <div className="flex flex-col gap-6">
      {/* Map Section */}
      {data.town_decay_rates &&
        Object.keys(data.town_decay_rates).length > 0 && (
      ...
      <div className="space-y-6">
        ...
        <div className="bg-orange-50 dark:bg-orange-900/20 border border-orange-200 dark:border-orange-800 rounded-lg p-4">
          <h3 className="font-semibold text-orange-900 dark:text-orange-100 mb-2">
            ⚠️ The Maturity Cliff
          </h3>
          <ul className="text-sm text-orange-800 dark:text-orange-200 space-y-1">
            <li>
              • <strong>70-80 years remaining:</strong>{" "}
              {data.insights.maturity_cliff.discount}% discount,{" "}
              {data.insights.maturity_cliff.annual_rate}% annual decay
            </li>
            <li>
              • <strong>Best value:</strong> {data.insights.best_value.band} at{" "}
              {data.insights.best_value.discount}% discount with high liquidity
            </li>
            <li>
              • <strong>Safe zone:</strong> {data.insights.safe_zone.band} for
              long-term holds
            </li>
          </ul>
        </div>
        ...
        {/* Results */}
        <div
          className={`border rounded-lg p-4 ${riskConfig.bg} ${riskConfig.border}`}
        >
