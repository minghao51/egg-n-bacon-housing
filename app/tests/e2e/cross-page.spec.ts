...
test.describe("Cross-Page Navigation", () => {
  ...
  test("should navigate from dashboard to analytics and back", async ({
    page,
  }) => {
  ...
  test("should navigate from analytics to persona and back", async ({
    page,
  }) => {
    await page.goto("/analytics/");
    await page
      .getByRole("link", { name: /First-Time Buyer/i })
      .first()
      .click();
  ...
  test("should navigate between different dashboard pages", async ({
    page,
  }) => {
    await page.goto("/dashboard");

    await page
      .getByRole("link", { name: /Explore Areas/ })
      .first()
      .click();
    await expect(page).toHaveURL(/dashboard\/map/);

    await page
      .getByRole("link", { name: /Decision Tools/ })
      .first()
      .click();
    await expect(page).toHaveURL(/dashboard\/trends/);

    await page
      .getByRole("link", { name: /Discover Segments/ })
      .first()
      .click();
    await expect(page).toHaveURL(/dashboard\/segments/);

    await page
      .getByRole("link", { name: /Compare Areas/ })
      .first()
      .click();
...
test.describe("Dark Mode Toggle", () => {
  test("should have dark mode toggle in sidebar", async ({ page }) => {
    ...
    const toggle = page.locator(
      'button[class*="toggle"], [aria-label*="dark"], [class*="DarkMode"]',
    );
