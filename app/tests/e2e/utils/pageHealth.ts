...
function isIgnorableFailedRequest(
  url: string,
  status: number,
  patterns: IgnorePattern[] = [],
): boolean {
  if (
    status === 404 &&
    /\/(?:favicon(?:-\d+x\d+)?\.(?:ico|png|svg)|apple-touch-icon(?:-precomposed)?\.png)(?:\?|$)/.test(
      url,
    )
  ) {
    return true;
  ...
  return (
    matchesPattern(`${status} ${url}`, patterns) ||
