...
test.describe("Persona Pages", () => {
  ...
  test.describe("Investor Persona", () => {
    ...
    test("should load investor persona page", async ({ page }) => {
      ...
      await expect(
        page.getByRole("heading", { name: "Property Investor" }),
      ).toBeVisible();
    });
  ...
  test.describe("Upgrader Persona", () => {
    ...
    test("should load upgrader persona page", async ({ page }) => {
      ...
      await expect(
        page.getByRole("heading", { name: "Upsizer / Upgrader" }),
      ).toBeVisible();
    });
  ...
  test("should not have critical console errors on persona pages", async ({
    page,
  }) => {
    const personas = ["first-time-buyer", "investor", "upgrader"];
    for (const persona of personas) {
      await expectPageToLoadWithoutErrors(
        page,
        `/analytics/personas/${persona}`,
      );
    }
  });
});
